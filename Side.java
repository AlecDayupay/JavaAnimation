...
/**
    Side.java implements DrawingObject.java interface. Side.java uses an
    instance of Path2D.Double() to draw a 4-sided shape that off-sets
    the right side by slant.
*/

import java.awt.*;
import java.awt.geom.*;

public class Side implements DrawingObject{
    private double x, y, width, length, slant;
    private Color color;
    private double rotation = 0;

    public Side(double x, double y, double width, double length, double slant, Color color){
        this.x = x;
        this.y = y;
        this.width = width;
        this.length = length;
    ...
    public void draw(Graphics2D g2d){
        ...
        g2d.rotate(Math.toRadians(rotation));
            ...
            p.moveTo(x,y);
            p.lineTo(x+width, y+slant);
            p.lineTo(x+width, y+slant+length);
            p.lineTo(x, y+length);
            g2d.setPaint(color);
            g2d.fill(p);
